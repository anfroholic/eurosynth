...
# Baked-in SystemVerilog init for src/neural_osc.sv. This makes the weight memory
# SYNTHESIS-SAFE: instead of $readmemh-ing neural_weights.hex (a path that only
# resolves at sim time, cwd=repo root, and FAILS under a synthesis run dir), the
# RTL `\`include`s this generated file, whose `initial` block writes the SAME
# values into the `mem` reg array. Identical bit-for-bit to neural_weights.hex.
INIT_SVH_PATH = os.path.join(os.path.dirname(os.path.abspath(__file__)),
                             os.pardir, "src", "neural_weights_init.svh")

# Name + index range of the weight/LUT reg array in src/neural_osc.sv. The
# generated initial block assigns mem[0 .. MEM_WORDS-1].
MEM_ARRAY = "mem"
...
def write_init_svh(lut, words, path=INIT_SVH_PATH):
    """Emit src/neural_weights_init.svh: a SystemVerilog `initial` block that
    writes the SAME memory image (LUT then weights) into the RTL's `mem` reg
    array, one word per assignment. `\`include`d by src/neural_osc.sv in place of
    the synthesis-unsafe $readmemh, so power-on/reset-default values are baked
    into the RTL with no runtime file dependency. Values are bit-for-bit
    identical to neural_weights.hex (same hex words, same order)."""
    mem_words = list(lut) + list(words)
    n = len(mem_words)
    width = len("%d" % (n - 1))                  # pad indices for tidy alignment
    with open(path, "w", newline="\n") as f:
        f.write("// SPDX-License-Identifier: Apache-2.0\n")
        f.write("//\n")
        f.write("// GENERATED by models/neural_train.py -- do not edit by hand.\n")
        f.write("//\n")
        f.write("// Baked power-on/reset-default contents of the neural_osc weight/LUT\n")
        f.write("// memory (`%s` reg array): sine LUT (words 0..%d) then the %d MLP\n"
                % (MEM_ARRAY, LUT_DEPTH - 1, len(words)))
        f.write("// weight/bias words (%d..%d). Values are bit-for-bit identical to\n"
                % (LUT_DEPTH, n - 1))
        f.write("// models/neural_weights.hex. `\\`include`d by src/neural_osc.sv so the\n")
        f.write("// memory needs no runtime $readmemh (synthesis-safe).\n")
        f.write("//\n")
        f.write("// %d words total (%d-word LUT + %d weight/bias words).\n"
                % (n, LUT_DEPTH, len(words)))
        f.write("initial begin\n")
        for i, v in enumerate(mem_words):
            f.write("    %s[%*d] = 16'h%04x;\n"
                    % (MEM_ARRAY, width, i, v & 0xFFFF))
        f.write("end\n")
...
def main():
    ...
    print("  init include  = %s  (baked RTL init, identical values)"
          % os.path.normpath(INIT_SVH_PATH))
